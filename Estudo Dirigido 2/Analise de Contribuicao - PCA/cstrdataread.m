...
% filename = 'X.csv';
% sepchar = ';';
% namenormalclass = 'normal';
% data = cstrdataread( 'X.csv', ';', 'normal' )
...
fclose(f);
n = k-numcommentlines-1; % n = numero de linhas
...
while ~feof(f)
    ...
    % Substituindo strsplit %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    last_sepchar = 0;
    c = 1;
    for i=1:length(l)
        if l(i)==sepchar
            cols{c}=l(last_sepchar+1:i-1);
            last_sepchar = i;
            c = c+1;
        end
    end
