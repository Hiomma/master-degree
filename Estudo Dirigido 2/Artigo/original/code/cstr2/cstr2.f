...
C
      ...
      BETA2       =     45000.0D0
      CA0         =     20.0D0
      CA          =     2.850D0  ! CA+CB+CC = 19.9866 --- Shouln't it be 20.0?
      CB          =     17.114D0
      CCNOM       =     0.0226D0
      CC          =     CCNOM
      CP          =     4.20D0
...
60    CONTINUE
C
      write(*,*) 'ITERATION=',iter,'TIME=',TIME,'M=',M,' S0=',S0,
     1   ' F(1)=',F(1),' F(2)=',F(2),
     1   ' F(3)=',F(3),' classstr=>>>',classstr,'<<<'
C
100   CONTINUE
C
C ADD NOISE
...
270   CONTINUE
     ...
     1               (MEAS(I,2),csvsep,I=1,18),' ',classstr
C
...
C ZCOUNT                - COUNTER FOR PRINTING OUTPUT
     ...
     1    'Closing at TIME=',TIME,'  Iteration=',iter,
...
C Add noise to a recently updated variable
       ...
       REAL*8 var,varold
       REAL*8 DSEED,RAND(1),NOISESCAL
C      INTEGER I

C      return ! Do not add any noise
C      NOISESCAL = 1D-4
C      NOISESCAL = 0D0
C      varold = var
C      CALL GGNML(DSEED,1,RAND)
C      var=var * (1.0D0 + NOISESCAL*RAND(1))
